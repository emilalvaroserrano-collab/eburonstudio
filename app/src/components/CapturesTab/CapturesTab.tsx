...
export function CapturesTab() {
  ...
  useEffect(() => {
    if (!platform.metadata.isTauri || typeof window === 'undefined' || !('__TAURI_INTERNALS__' in window)) return;
  ...
  const handleExportAudio = async () => {
    ...
    try {
      ...
      await platform.filesystem.saveFile(filename, blob, [{ name: 'Audio', extensions: ['wav'] }]);
  ...
  const handleExportTranscript = async () => {
    ...
    try {
      ...
      await platform.filesystem.saveFile(filename, blob, [{ name: 'Text', extensions: ['txt'] }]);
      exportToastSuccess(filename);
    } catch (err) {
      exportToastError(err);
    }
  ...
  const handleExportMarkdown = async () => {
    ...
    try {
      ...
      await platform.filesystem.saveFile(filename, blob, [{ name: 'Markdown', extensions: ['md'] }]);
      exportToastSuccess(filename);
    } catch (err) {
      exportToastError(err);
    }
